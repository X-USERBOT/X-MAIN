...
async def kk(event):
    ...
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            ...
            await edit_or_reply(x_file, f"Output Too Large. This is the file for the list of plugins in bot.\n\n**BY :-** {X_USER}")
            await event.delete()


@bot.on(x_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
...
async def send(event):
    ...
    omk = f"**• Plugin name ≈** `{input_str}`\n**• Uploaded by ≈** {x_mention}\n\n⚡ **[ℓєgєη∂αяу αƒ χ-υѕєявσт]({chnl_link})** ⚡"
    the_plugin_file = "./xbot/plugins/{}.py".format(input_str)
    ...
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(  # pylint:disable=E0602
                ...
                "./xbot/plugins/"  # pylint:disable=E0602
            )
...
@bot.on(x_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
...
async def uninstall(kraken):
    ...
    dir_path =f"./xbot/plugins/{shortname}.py"
    try:
...
@bot.on(x_cmd(pattern=r"unload (?P<shortname>\w+)$"))
...
@bot.on(x_cmd(pattern=r"load (?P<shortname>\w+)$"))
...
CmdHelp("core").add_command(
  ...
  "cmds", None, "Gives out the list of modules in xBot."
).add_warning(
