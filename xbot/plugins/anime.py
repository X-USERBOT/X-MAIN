...
@bot.on(x_cmd(pattern="anime(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="anime(?: |$)(.*)", allow_sudo=True))
async def nope(hel_):
    x = hel_.pattern_match.group(1)
    if not x:
        if hel_.is_reply:
    ...
    troll = await bot.inline_query("AniFluidbot", f".anime {(deEmojify(x))}")
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".manga {(deEmojify(x))}")
...
@bot.on(x_cmd(pattern="character(?: |$)(.*)"))
...
async def nope(hel_):
    ...
    troll = await bot.inline_query("AniFluidbot", f".character {(deEmojify(x))}")
