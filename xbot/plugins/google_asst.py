...
@bot.on(x_cmd(pattern=r"trc", outgoing=True))
@bot.on(sudo_cmd(pattern=r"trc", allow_sudo=True))
async def _(x):
    if x.fwd_from:
        return
    await edit_or_reply(x, "**All The Language Codes Can Be Found** \n ⚡ [Here](https://t.me/P_4_PEEYUSH) ⚡", link_preview=False)


@bot.on(x_cmd(pattern="voice (.*)"))
