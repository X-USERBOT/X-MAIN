...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        ammoca_message = ""
        xxxsemx = await event.client.get_entity(event.sender_id)
        if xxxsemx.bot or xxxsemx.verified or xxxsemx.support:
            return
        xxxsemxm = f"[{get_display_name(xxxsemx)}](tg://user?id={xxxsemx.id})"
...
@bot.on(x_cmd(pattern=r"tagall (.*)", outgoing=True))
