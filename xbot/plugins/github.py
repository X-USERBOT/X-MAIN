...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        await eod(event, "`Please ADD Proper Github Repo Name of XBot`")
        return
    xbot = await eor(event, "Processing ...")
    ...
    try:
        ...
        await xbot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, xbot)


async def git_commit(file_name, xbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await xbot.edit("`File Already Exists`")
            create_file = False
    file_name = "xbot/plugins/" + file_name
    if create_file == True:
        ...
        try:
            ...
            await eod(xbot, "Cannot Upload Plugin")
    else:
        return await eod(xbot, "`Committed Suicide`")


@bot.on(x_cmd(pattern="github (.*)", outgoing=True))
