...
@bot.on(x_cmd(pattern="bigspam"))
@bot.on(sudo_cmd(pattern="bigspam", allow_sudo=True))
async def bigspam(lx):
    if not x.text[0].isalpha() and x.text[0] not in ("/", "#", "@", "!"):
        x_msg = x.text
        xbot_count = int(x_msg[9:13])
        x_spam = str(x.text[13:])
        for i in range(1, xbot_count):
            await x.respond(x_spam)
        await x.delete()
        await x.client.send_message(
                lg_id, f"#BIGSPAM \n\nBigspammed  `{x_count}`  messages !!"
        )
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. The XBot is known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add_info(
  "Spammers Commands"
).add_warning(
