...
async def lg1(xevent):
    ...
    if xevent.reply_to_msg_id:
        ...
        async for i in bot.iter_messages("@x_backgrounds", filter=InputMessagesFilterPhotos):
         PICS_STR.append(i)
        pic = random.choice(PICS_STR)
        logo_ = await pic.download_media()
    text = xevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
