...
from xbot.helpers import runner
from . import *
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "✅ Successfully updated χ-υѕєявσт!\n\nBot is restarting please wait for a minute."
    )
    args = [sys.executable, "-m", "xbot"]
    os.execle(sys.executable, *args, os.environ)
    return


@bot.on(x_cmd(outgoing=True, pattern=r"update(| now)$"))
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your **χ-υѕєявσт** !!")

    if force_update:
        await event.edit(
            "`Force-Updating χ-υѕєявσт. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your χ-υѕєявσт Is UpToDate**\n\n**Version :**  __{x_ver}__\n**Oɯɳҽɾ :**  {x_mention}")
    else:
        await event.edit("**Please set up**  `HEROKU_API_KEY`  **from heroku to update!**")
    return


@bot.on(x_cmd(outgoing=True, pattern=r"update build$"))
...
async def upstream(event):
    ...
    off_repo = "https://github.com/LiveToLife/X-Userbot"
    os.chdir("/app")
    git_x = f"rm -rf .git"
    try:
        await runner.runcmd(git_x)
    except BaseException:
        pass
    try:
        txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit(f"**χ-υѕєявσт Docker Build In Progress... Type** `{hl}ping`  **after 5 mins to check if Bot is working!**")
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your χ-υѕєявσт. Basically if you restart dyno it will go back to previous deploy."
).add_command(
  "update build", None, "Hard-Update Your χ-υѕєявσт. This won't take you back to your previous deploy. This will be triggered even if there is no changelog."
).add_info(
  "χ-υѕєявσт Updater."
).add_warning(
  "✅ Harmless Module."
