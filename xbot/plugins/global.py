...
import asyncio
from xbot.sql.gban_sql import is_gbanned, gbaner, ungbaner, all_gbanned
from xbot.sql import gmute_sql as gsql
from . import *


@bot.on(x_cmd(pattern=r"gban ?(.*)"))
@bot.on(sudo_cmd(pattern=r"gban ?(.*)", allow_sudo=True))
async def _(event):
    x = await eor(event, "`Gbanning...`")
    reason = ""
    if event.reply_to_msg_id:
        ...
        return await eod(x, "**To gban a user i need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == P_4_PEEYUSH:
        return await eod(x, "🥴 **You Taked Weed Or Liquior Seriously I will Gban My Creator ‽**")
    if str(userid) in DEVLIST:
        return await eod(x, "😑 **GBan my creator ?¿ Really‽**")
    ...
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                ...
                pass
    gbaner(userid)
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **Thanks Now You Are Gbanned 😪 Now Enjoy By ** {x_mention} **Gbanned Now Enjoy Fucki--ng Life 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **Is now GBanned by** {x_mention} **in**  `{chats}`  **Chats!! 😏**\n\n**📍 Also Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**🔰 Reason :**  `{reason}`"
    if Config.ABUSE == "ON":
...
@bot.on(x_cmd(pattern=r"ungban ?(.*)"))
@bot.on(sudo_cmd(pattern=r"ungban ?(.*)", allow_sudo=True))
async def _(event):
    x = await eor(event, "`Ungban in progress...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(x, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eod(x, "`User is not gbanned.`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
...
@bot.on(x_cmd(pattern=r"gkick ?(.*)"))
@bot.on(sudo_cmd(pattern=r"gkick ?(.*)", allow_sudo=True))
async def gkick(event):
    x = await eor(event, "`Kicking globally...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        userid = await get_user_id(event.pattern_match.group(1))
        ...
        return await eod(x, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == P_4_PEEYUSH:
        return await eod(x, "**You Taked Weed Or Liquior Seriously I will Gban My Creator ‽**")
    if str(userid) in DEVLIST:
        return await eod(x, "**😪 I'm not going to gkick my developer!!**")
...
@bot.on(x_cmd(pattern=r"gmute ?(\d+)?"))
...
@bot.on(x_cmd(outgoing=True, pattern=r"ungmute ?(\d+)?"))
