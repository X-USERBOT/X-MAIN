...
@bot.on(x_cmd(pattern="recognize ?(.*)", outgoing=True))
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    x = await eor(event, "recognizeing this media")
...
@bot.on(x_cmd(pattern="info ?(.*)", outgoing=True))
...
<b>⚡ <a href='https://t.me/X_User_Bott'>From DataBase of xBot</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@bot.on(x_cmd(pattern="chatinfo(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="chatinfo(?: |$)(.*)", allow_sudo=True))
async def info(event):
    if event.fwd_from:
        return
    x = await eor(event, "`Analysing the chat...`")
    ...
    try:
        ...
        print("Exception:", e)
        await eod(x, "`An unexpected error has occurred.`")
    return
...
@bot.on(x_cmd(pattern=r"users ?(.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="id$"))
@bot.on(sudo_cmd(pattern="id$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    x = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await x.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
