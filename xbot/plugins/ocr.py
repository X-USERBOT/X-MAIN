...
@bot.on(x_cmd(pattern="ocrlang", outgoing=True))
...
@bot.on(x_cmd(pattern=r"ocr (.*)", outgoing=True))
...
async def parse_ocr_space_api(event):
    ...
    try:
        ...
        await eod(x, "**Errors !!** \n`{}`\n**Report This to** {}\n\n`{}`".format(
                str(e), x_grp, json.dumps(test_file, sort_keys=True, indent=4)
            )
        )
    else:
        await x.edit("Read Document in {} seconds. \n{}".format(
                ProcessingTimeInMilliseconds, ParsedText
            )
        )
    os.remove(downloaded_file_name)
    await x.edit(ParsedText)
