...
@bot.on(x_cmd(pattern="autoname"))
async def _(event):
    if event.fwd_from:
        return
    x = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    while True:
        ...
        await x.edit(f"Auto Name has been started my Master")
        await bot.send_message(Config.LOGGER_ID, "#AUTONAME \n\nAutoname Started!!")


@bot.on(x_cmd(pattern="autobio"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(x_cmd(pattern="reserved", outgoing=True))
