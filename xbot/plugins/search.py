...
@bot.on(x_cmd(pattern="ytube (.*)", outgoing=True))
...
async def _(event):
    ...
    if response_api:
        await edit_or_reply(event, "Let me *YouTube** that for you:\n👉 [{}]({})\n`Thank me later 😉` ".format(
...
@bot.on(x_cmd(pattern="ddg (.*)", outgoing=True))
...
async def _(event):
    ...
    response_api = requests.get(sample_url).text
    if response_api:
        await edit_or_reply(event, "Let me **Duck Duck Go** that for you:\n👉 [{}]({})\n`Thank me later 😉` ".format(
                input_str, response_api.rstrip()
            )
        )
    else:
...
@bot.on(x_cmd(pattern="altn (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="var (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="lmlog (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="hacc (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="lmkp (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="gem (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="rchiv (.*)", outgoing=True))
