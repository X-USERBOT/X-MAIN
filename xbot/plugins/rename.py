...
@bot.on(x_cmd(pattern="rename (.*)", outgoing=True))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            await x.edit("Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
            )
        else:
            await eod(x, "Error Occurred\n {}".format(input_str))
    else:
        await eod(x, f"Syntax `{hl}rename file.name` as reply to a Telegram media")


@bot.on(x_cmd(pattern="rnupload (.*)", outgoing=True))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await x.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(
                    ms_one, ms_two
                )
            )
        else:
        ...
        await x.edit("Syntax // `{}rnupload file.name` as reply to a Telegram media".format(hl))


@bot.on(x_cmd(pattern="rnsupload (.*)", outgoing=True))
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                await bot.send_file(
                    ...
                    downloaded_file_name,
                    thumb=thumb,
                    caption="reuploaded by [xBot](https://t.me/xbot_official_chat)",
                    force_document=False,
                    allow_cache=False,
                    reply_to=event.message.id,
                    attributes=[
            ...
            await eod(x, "File Not Found {}".format(input_str))
    else:
        await x.edit(
