...
async def _(event):
    ...
    async with event.client.action(event.chat_id, "typing"):
        await event.client.send_message(
            ...
            message="""{}""".format(random.choice(XBOT)),
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        event = await edit_or_reply(event, "Aagya Swad Merese Fight Krke Maderchod")
        que[e] = []
        qeue = que.get(e)
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"Reply Raid Activated On..{username}")
    else:
        user = event.pattern_match.group(1)
        event = await edit_or_reply(event, "Bhosdi Ke Jada Bhosadpatti Mat Kar Tu Mera nazayaz aullad ")
        ...
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"Ja Teri Maa Ko Puchkhe Aa Tereko Kisne Paida Kiya Tha. {username}")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        c = b.first_name
        username = f"[{c}](tg://user?id={e})"
        event = await edit_or_reply(event, "Mene Jis Din Tere Maa Ko Choda Na Upr Se Niche Se Piche Se Doggy Style Sab Kiya Puchlena ")
        queue = que.get(e)
        queue.pop(0)
        await event.edit(f"Ab Aagya Na Swad Bhosdi Ke")
    else:
        user = event.pattern_match.group(1)
