...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    xevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(xevent, "`Please unblock `@DrWebBot `and try again`")
            return
...
@bot.on(x_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(x_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    xevent = await eor(event, "...")
    ...
    try:
        ...
        await xevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await xevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await xevent.delete()


@bot.on(x_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await eod(x, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(x_cmd(pattern="currency (.*)"))
...
@bot.on(x_cmd(pattern="currencies$"))
...
@bot.on(x_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    xevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await xevent.edit(output_str, link_preview=True)
    else:
        await eod(xevent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(x_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(x_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for XBot", ".currency 10 usd inr"
).add_command(
