...
@bot.on(x_cmd(pattern=r"tweet(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="tweet(?: |$)(.*)", allow_sudo=True))
async def nope(semx):
    xbot = semx.pattern_match.group(1)
    okvai = await eor(semx, "Trying to tweet for you...")
    if not xbot:
        if semx.is_reply:
            (await semx.get_reply_message()).message
        else:
            await eod(semx, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(xbot))}")
    await tweeter[0].click(
        semx.chat_id,
        reply_to=semx.reply_to_msg_id,
        silent=True if semx.is_reply else False,
        hide_via=True,
    )
    await semx.delete()


@bot.on(x_cmd(pattern=r"trump(?: |$)(.*)"))
...
CmdHelp("tweets").add_command(
  ...
  "trump", "<text>/<reply>", "Tweet with Mr. DoLand Trump"
).add_command(
