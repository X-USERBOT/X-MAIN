...
@bot.on(x_cmd(pattern=r"inviteall ?(.*)"))
...
async def get_users(event):
    ...
    if he_ll == "@X_User_Bot":
        return await x.edit("Restricted to invite users from there.")
    elif he_ll == "@X_Discussion":
        return await x.edit("Restricted to invite users from there.")
    elif he_ll == "@SharkModsDiscussion":
        return await x.edit("Restricted to invite users from there.")
    kraken = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await x.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await x.edit("**INVITING USERS !!**")
