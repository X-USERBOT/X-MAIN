...
async def all_groups_id(x):
    xgroups = []
    async for dialog in x.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            xgroups.append(entity.id)
    return xgroups


@bot.on(x_cmd(pattern="frwd$"))
