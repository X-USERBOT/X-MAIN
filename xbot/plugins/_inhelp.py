...
alive_txt = """
**⚜️ ℓєgєη∂αяу αƒ χ-υѕєявσт ιѕ σηℓιηє ⚜️**
{}
**🏅 𝙱𝚘𝚝 𝚂𝚝𝚊𝚝𝚞𝚜 🏅**
    ...
    buttons.append(
        [
            custom.Button.inline(
               f"◀️ Back {x_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"• ❌ •", data="close"
            ),
            custom.Button.inline(
               f"{x_emoji} Next ▶️", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query == "xbot_help":
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚡ ℓєgєη∂αяу αƒ χ-υѕєявσт ⚡**",
                buttons=[
                    [Button.url("📑 Repo 📑", "https://t.me/x_discussion")],
                    [Button.url("🚀 Deploy 🚀", "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FX-USERBOT%2FX-MAIN&template=https%3A%2F%2Fgithub.com%2FX-USERBOT%2X-MAIN")],
                ],
            )
