...
async def _(event):
    ...
    xevent = await eor(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    xuser = await event.client.get_me()
    if xuser.username:
        curruser = xuser.username
    else:
        curruser = "xbot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    xevent = await eor(event, "`Running ...`")
    ...
    if exc:
        ...
        evaluation = "Success"
    final_output = f"•  Eval : \n`{cmd}` \n\n•  Result : \n`{evaluation}` \n"
    await eor(xevent, "**Eval Command Executed. Check out LOGGER for result.**")
    await event.client.send_message(
        lg_id,
...
@bot.on(x_cmd(pattern="bash ?(.*)", outgoing=True))
...
CmdHelp("evaluators").add_command(
  ...
  "exec", "<command>", "Execute a Terminal command on XBot server and shows details"
).add_command(
