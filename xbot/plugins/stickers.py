...
xbot = Config.STICKER_PACKNAME


@bot.on(x_cmd(outgoing=True, pattern="kang"))
...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            x = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packname = f"{user.username}_Xbot_{pack}"
        packnick = (
            f"{xbot} Vol.{pack}"
            if xbot
            else f"@{user.username}'s xBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await x.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(x_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(x_cmd(pattern=r"editst ?(.*)", outgoing=True))
...
async def sticklet(event):
    ...
    FONT_FILE = await get_font_file(event.client, "@XFonts")
...
@bot.on(x_cmd(pattern="waifu(?: |$)(.*)", outgoing=True))
