...
@bot.on(x_cmd(pattern="newfed ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    x_input = event.pattern_match.group(1)
    chat = "@MissRose_Bot"
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {x_input}")
...
@bot.on(x_cmd(pattern="renamefed ?(.*)"))
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {x_input}")
              response = await response 
          except YouBlockedUserError: 
...
@bot.on(x_cmd(pattern="fstat ?(.*)"))
@bot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    x = await eor(event, "`Collecting fstat....`")
    thumb = x_logo
    ...
    if lavde == "":
        ...
        async with bot.conversation(fbot) as conv:
            try:
                ...
                await x.edit("`Please Unblock` @MissRose_Bot")


@bot.on(x_cmd(pattern="fedinfo ?(.*)"))
@bot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    x = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with bot.conversation(fbot) as conv:
        try:
