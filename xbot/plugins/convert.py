...
@bot.on(x_cmd(pattern="stoi$"))
@bot.on(sudo_cmd(pattern="stoi$", allow_sudo=True))
async def _(x):
    if x.fwd_from:
        return
    reply_to_id = x.message.id
    if x.reply_to_msg_id:
        reply_to_id = x.reply_to_msg_id
    event = await eor(x, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await x.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await x.client.send_file(
                event.chat_id,
...
@bot.on(x_cmd(pattern="gif$"))
@bot.on(sudo_cmd(pattern="gif$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    xreply = await event.get_reply_message()
    if not xreply or not xreply.media or not xreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if xreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    xevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with bot.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await bot.send_file(chat, xreply.media)
            response = await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await xevent.edit("`This file is not supported`")
            xresponse = response if response.media else await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            xfile = Path(await event.client.download_media(xresponse, "./temp/"))
            xgif = Path(await unzip(xfile))
            semx = await bot.send_file(
                event.chat_id,
                xgif,
            ...
            await bot(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=semx.media.document.id,
                        access_hash=semx.media.document.access_hash,
                        file_reference=semx.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            await xevent.delete()
            for files in (xgif, xfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await xevent.edit("Unblock @tgstogifbot")
            return


@bot.on(x_cmd(pattern="nfc ?(.*)"))
