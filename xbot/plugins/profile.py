...
@bot.on(x_cmd(pattern="offline$", outgoing=True)) 
async def _(event):
    if event.fwd_from:
        return
    user_it = "me"
...
@bot.on(x_cmd(pattern="online$", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    user_it = "me"
...
@bot.on(x_cmd(outgoing=True, pattern="username (.*)"))
...
@bot.on(x_cmd(outgoing=True, pattern="count$"))
...
@bot.on(x_cmd(outgoing=True, pattern=r"delpfp"))
...
@bot.on(x_cmd(pattern="myusernames$"))
async def _(event):
    if event.fwd_from:
        return
