...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        execl(executable, executable, "bash", "xbot")
...
async def variable(xbot):
    ...
    if exe == "get":
        ...
        event = await eor(x, "Setting Heroku Variable...")
        variable = x.pattern_match.group(2)
        if not variable:
            return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        value = x.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = x.pattern_match.group(2).split()[1]
            except IndexError:
                return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        await asyncio.sleep(1.5)
        if variable in heroku_var:
        ...
        event = await eor(x, "Getting info to delete Variable")
        try:
...
@bot.on(x_cmd(pattern="usage(?: |$)", outgoing=True))
@bot.on(sudo_cmd(pattern="usage(?: |$)", allow_sudo=True))
async def dyno_usage(x):
    if x.fwd_from:
        return
    event = await edit_or_reply(x, "`Processing...`")
    useragent = (
...
@bot.on(x_cmd(pattern="logs$"))
@bot.on(sudo_cmd(pattern="logs$", allow_sudo=True))
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {x_grp} for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku AppName & API Key are filled correct. Visit {x_grp} for help.", link_preview=False)
   # event = await eor(dyno, "Downloading Logs...")
    x_data = app.get_log()
    await eor(dyno, x_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :**  {x_mention}\n\n🚀** Pasted**  ")
