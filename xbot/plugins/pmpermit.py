...
from . import *
from xbot.sql import pmpermit_sql as pm_sql


WARN_PIC = Config.PMPERMIT_PIC or "https://telegra.ph/file/ae53a9b45f87e9b493241.jpg"
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
PM_ON_OFF = Config.PM_PERMIT
CSTM_PMP = Config.CUSTOM_PMPERMIT or "**You Have Trespassed To My Master's PM!\nThis Is Illegal And Regarded As Crime So Please Wait Or Get Blocked.**"
XBOT_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"
XBOT_FIRST = (
    "**🔥 χ-υѕєявσт ρяιναтє ѕє¢υяιту ρяσтσ¢σℓ 🔥**\n\nThis is to inform you that "
    "{} is currently unavailable.\nThis is an automated message.\n\n"
    "{}\n\n**Please Choose Why You Are Here!!**".format(x_mention, CSTM_PMP)
)

@bot.on(x_cmd(pattern="block$"))
async def approve_p_m(event):
    ...
    if event.is_private:
        ...
        if str(reply_s.sender_id) in DEVLIST:
            await event.edit("**I can't Block My Creator My Boss !!**")
            return
        if pm_sql.is_approved(event.chat_id):
            pm_sql.disapprove(event.chat_id)
        await event.edit("Go fuck yourself Now Enjoy !! \n\n**Blocked** [{}](tg://user?id={})".format(firstname, reply_s.sender_id))
...
if PM_ON_OFF != "DISABLE":
    ...
    @bot.on(x_cmd(pattern="(a|approve|allow)$"))
    async def approve(event):
        ...
        if event.is_private:
            ...
            if not pm_sql.is_approved(event.chat_id):
                ...
                await event.edit(
                    "Approved to Chat [{}](tg://user?id={})".format(firstname, event.chat_id)
                )
                await asyncio.sleep(3)
                await event.delete()
            elif pm_sql.is_approved(event.chat_id):
                hel_ = await event.edit('Already In Approved List! Boss! !')
                await asyncio.sleep(3)
                await hel_.delete()
        elif event.is_group:
            reply_s = await event.get_reply_message()
    ...
    @bot.on(x_cmd(pattern="(da|disapprove|disallow)$"))
    ...
    @bot.on(x_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
...
CmdHelp("pm_permit").add_command(
  "allow", "<in pm>", "Approves the user in which pm command is used."
).add_command(
  "disallow", "<in pm>", "Disapprove User to PM you."
).add_command(
  "block", "<in pm>", "Blocks the user"
).add_command(
  "listapproved", None, "Sends the list of all users approved by χ-υѕєявσт"
).add_info(
  "PM SECURITY"
).add_warning(
  "✅ Harmless Module."
