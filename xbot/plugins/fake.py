...
async def _(event):
    ...
    if response.status_code == 200:
      with open("xBOT.jpg", 'wb') as f:
        f.write(response.content)
    
    captin = f"Fake Image By {x_mention}"
    fole = "xBOT.jpg"
    await bot.send_file(event.chat_id, fole, caption=captin)
    await event.delete()
    os.system("rm /root/xbot/xBOT.jpg ")


@bot.on(x_cmd(pattern="fake ?(.*)"))
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1337343465:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ [Peeyush](https://t.me/P_4_PEEYUSH) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
